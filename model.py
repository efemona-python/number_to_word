class GenerateWord:
    ...
    def generate_words(self, number):
        ...
        if int(number) < 20:
            self.number_in_words = self.less_than_twenty[int(number)]
        elif int(number) == 0:
            self.number_in_words = "zero"
        else:
            for i in number_range:
                ...
                if self.count % 3 == 1:
                    ...
                    #             the number is less than 1000
                        ...
                        if self.multiplier_count < 1:
                            self.number_in_words.append(self.less_than_twenty[int(n)])
                        #             the unit of the multiplier is zero
                        elif int(n) == 0:
                            self.number_in_words.append(
                ...
                #       check if tens
                    ...
                    if int(n) < 2:
                        self.number_in_words.append(self.less_than_twenty[int(n + n_temp)])
                    else:
                        self.number_in_words.append(self.tens[int(n)] + "-")
                ...
                #       check if hundreds
                    ...
                    if int(n) == 0:
                        ...
                        self.number_in_words.append(self.less_than_twenty[int(n)] + " hundred and ")
                    #           the next value after HTU will be a multiplier
                    self.multiplier_count += 1

                self.count += 1
        return "".join(self.number_in_words[::-1]).capitalize().rstrip('-')
